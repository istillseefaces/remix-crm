...
export const DatePicker: React.FC<DatePickerProps> = ({
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-mono transition-all cursor-pointer select-none border ${
            isLight
              ...
              : 'bg-[var(--surface-secondary)] hover:bg-[var(--hover)] text-zinc-200 border-white/[0.06]'
        ...
        <button
          ...
          className={`w-full px-3 py-2 ${value && allowClear ? 'pr-8' : ''} rounded-lg border text-xs flex items-center justify-between gap-2 transition cursor-pointer select-none font-mono ${
            isLight
              ...
              : 'bg-[var(--surface-secondary)] text-zinc-100 border-white/[0.08] hover:border-white/20'
  ...
  const popoverContent = isOpen ? (
    <div
      ...
      className={`rounded-2xl shadow-2xl overflow-hidden border backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-250 p-3 select-none ${
        isLight
          ? 'bg-white/98 border-black/[0.1] text-zinc-900 shadow-black/15'
          : 'bg-[var(--surface)]/98 border-white/10 text-white shadow-black/90'
      }`}
    >
